...
// symbol> 
void
ide_counters(int array[3], int devices[64], int blocks[64]){
  struct buf *b;
  ...
  for(b=idequeue; b; b=b->qnext) {
    ...
    if(b->flags & B_DIRTY)
      write_counter++;
    devices[waiting_counter]=b->dev;
    blocks[waiting_counter]=b->blockno;
    waiting_counter++;
  }
  array[0]=waiting_counter;
  array[1]=read_counter;
  array[2]=write_counter;
}